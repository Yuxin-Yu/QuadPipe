...
`endif
  ...
  assign ACIN_in = ACIN;
  /* -------------- ALUMODE -------------- */
assign ALUMODE_in[0] = ALUMODE[0] ^ IS_ALUMODE_INVERTED_REG[0];
assign ALUMODE_in[1] = ALUMODE[1] ^ IS_ALUMODE_INVERTED_REG[1];
assign ALUMODE_in[2] = ALUMODE[2] ^ IS_ALUMODE_INVERTED_REG[2];
assign ALUMODE_in[3] = ALUMODE[3] ^ IS_ALUMODE_INVERTED_REG[3];

/* -------------- A -------------- */
assign A_in = A;   // 30 位向量一次性赋值，等价于逐位 (A[i]===1'bz)||A[i] 去掉三态判断

/* -------------- B -------------- */
assign B_in = B;   // 18 位向量

/* -------------- C -------------- */
assign C_in = C;   // 48 位向量

/* -------------- D -------------- */
assign D_in = D;   // 27 位向量

/* -------------- CARRYINSEL -------------- */
assign CARRYINSEL_in[0] = CARRYINSEL[0];
assign CARRYINSEL_in[1] = CARRYINSEL[1];
assign CARRYINSEL_in[2] = CARRYINSEL[2];

/* -------------- CARRYIN -------------- */
assign CARRYIN_in = CARRYIN ^ IS_CARRYIN_INVERTED_REG;
...
/* -------------- INMODE -------------- */
assign INMODE_in[0] = INMODE[0] ^ IS_INMODE_INVERTED_REG[0];
assign INMODE_in[1] = INMODE[1] ^ IS_INMODE_INVERTED_REG[1];
assign INMODE_in[2] = INMODE[2] ^ IS_INMODE_INVERTED_REG[2];
assign INMODE_in[3] = INMODE[3] ^ IS_INMODE_INVERTED_REG[3];
assign INMODE_in[4] = INMODE[4] ^ IS_INMODE_INVERTED_REG[4];

/* -------------- OPMODE -------------- */
assign OPMODE_in[0] = OPMODE[0] ^ IS_OPMODE_INVERTED_REG[0];
assign OPMODE_in[1] = OPMODE[1] ^ IS_OPMODE_INVERTED_REG[1];
assign OPMODE_in[2] = OPMODE[2] ^ IS_OPMODE_INVERTED_REG[2];
assign OPMODE_in[3] = OPMODE[3] ^ IS_OPMODE_INVERTED_REG[3];
assign OPMODE_in[4] = OPMODE[4] ^ IS_OPMODE_INVERTED_REG[4];
assign OPMODE_in[5] = OPMODE[5] ^ IS_OPMODE_INVERTED_REG[5];
assign OPMODE_in[6] = OPMODE[6] ^ IS_OPMODE_INVERTED_REG[6];
assign OPMODE_in[7] = OPMODE[7] ^ IS_OPMODE_INVERTED_REG[7];
assign OPMODE_in[8] = OPMODE[8] ^ IS_OPMODE_INVERTED_REG[8];

/* -------------- RST* -------------- */
assign RSTALLCARRYIN_in = RSTALLCARRYIN ^ IS_RSTALLCARRYIN_INVERTED_REG;
assign RSTALUMODE_in    = RSTALUMODE    ^ IS_RSTALUMODE_INVERTED_REG;
assign RSTA_in          = RSTA          ^ IS_RSTA_INVERTED_REG;
assign RSTB_in          = RSTB          ^ IS_RSTB_INVERTED_REG;
assign RSTCTRL_in       = RSTCTRL       ^ IS_RSTCTRL_INVERTED_REG;
assign RSTC_in          = RSTC          ^ IS_RSTC_INVERTED_REG;
assign RSTD_in          = RSTD          ^ IS_RSTD_INVERTED_REG;
assign RSTINMODE_in     = RSTINMODE     ^ IS_RSTINMODE_INVERTED_REG;
assign RSTM_in          = RSTM          ^ IS_RSTM_INVERTED_REG;
assign RSTP_in          = RSTP          ^ IS_RSTP_INVERTED_REG;
